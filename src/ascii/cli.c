...
char get_cell_char(struct matrix *m, int row, int col)
{
...
void print_map(struct matrix *m, int label_lines, int print_references)
{
    ...
    "  Referencias:                                      \n"
    "   [#] obstáculos            ┃ [*] pasos del camino \n"
    "   [@] celdas de peso mayor  ┃ [I] punto de inicio  \n"
    "   [%] celdas de peso medio  ┃ [D] punto de fín     \n"
    "   [&] celdas de peso menor  ┃                      \n";
    ...
    ptr += sprintf(ptr, "%c", BORDER_CORNER);

    for (int i = 0; i < m->width; i++)
    {
        ptr += sprintf(ptr, "%c", BORDER_HORIZONTAL);
    }
    ptr += sprintf(ptr, "%c\n", BORDER_CORNER);
    ...
    for (int i = 0; i < m->height; i++)
    {
        ...
        ptr += sprintf(ptr, "%c", BORDER_VERTICAL);

        for (int j = 0; j < m->width; j++)
        {
            ptr += sprintf(ptr, "%c", get_cell_char(m, i, j));
        }
        ptr += sprintf(ptr, "%c\n", BORDER_VERTICAL);
    }
